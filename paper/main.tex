...
\textbf{Fairness-constrained hybrid.} Let $g(i)$ be the candidate group (e.g., Ethnicity, Distance to Treatment Center, or Sex). With $p_g$ the group's waitlist share and $a_g(t)$ its allocation share so far at step $t$,
\begin{equation}
...
\noindent Synthetic data and simplified survival assumptions; omitted clinical factors (notably CPRA, which measures donor-recipient antibody incompatibility and grants national priority to highly sensitized patients in practice); risk of bias when using group-aware adjustments; non-deployability without rigorous validation. We enforce ABO blood type compatibility (O donors $\rightarrow$ all types; A $\rightarrow$ A/AB; B $\rightarrow$ B/AB; AB $\rightarrow$ AB only) but focus on policy effects rather than achieving clinical precision, which would require CPRA modeling.
